...
#define GPIO_180 18
#define DEV_NAME "test_motor180_dev"
...
int servo180_close(struct inode *pinode, struct file *pfile){
  ...
  gpio_free(GPIO_180); // Release the pin
  return 0;
}

void turn180_servo(int mode){
  int i;
...
ssize_t servo180_write(struct file *pfile, const char __user *buffer, size_t length, loff_t *offset){ 
  ...
  if(copy_from_user(&get_msg,buffer,length)<0){
    printk(KERN_ALERT "[SERVO180] Write error\n");
    return -1;
  }

  if(get_msg=='0'){          //Turn 180 degrees
    turn_servo(0);
...
int __init servo180_init(void){
  ...
  printk("[SERVO180] Initialize SERVO180_dev major number = %d\n",major);
  return 0;
}
